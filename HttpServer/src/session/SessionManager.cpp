...
{   
    ...
    if (!sessionId.empty()) //正确提取sessionID
    {
        session = storage_->load(sessionId);
    }

    if (!session || session->isExpired()) //没有对应会话或原有会话已经过期
    {
        sessionId = generateSessionId();
        session = std::make_shared<Session>(sessionId, this); //（）里是构造函数所需参数
